...
```{r}
  get_cv_estimate = function(gene='BRCA1'){
    ...
    bx_DATA_SPLITS = k_fold_split_data(bx_input)  # change to k-fold CV, 2020-05-28
    ...
    for(v in 1:v_fold_nest) {
        ...
        write.csv(preds, file=paste0(gene,'_res.fold',v,'.csv'))
    }
    return(preds)
  }
...
if (train==TRUE){
  ...
  for (v in 1:v_fold_nest){
    b1_cv = NULL
    b2_cv = NULL
    b1 = read.csv(paste0('BRCA1_res.fold',v,'.csv'))
    b1_cv = rbind(b1_cv, b1)
    b2 = read.csv(paste0('BRCA2_res.fold',v,'.csv'))
...
if (train==TRUE){
  ...
  for (i in 1:v_fold_nest){
    tmp = read.csv(paste0('BRCA2_res.fold',i,'.csv'))
...
get_res = function(input_data, model){
  ...
  threshold = as.data.frame(t(d))$threshold
