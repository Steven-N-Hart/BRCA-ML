...
get_res = function(input_data, model){
  ...
  d = t(as.data.frame(coords(roc_obj, "best", ret=c("threshold","sensitivity", "specificity", "tp", "tn", "fp", "fn"))))
  rownames(d)=NULL
  d = as.data.frame(d)
...
p1 = res_roc_1$ROC %>%
  #filter(auc > 0.8) %>%
  #ggplot(aes(y=sensitivities, x=1-specificities, color = paste0(Model,"=", round(auc,3)))) +
...
Save table S2.
```{r write-table}
write.table(rbind(b1_PREDICTIONS, b2_PREDICTIONS), file='results/TableS2.tsv', sep='\t', row.names = FALSE)
```
